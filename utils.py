...
USAGE_FILE = os.getenv("USAGE_FILE", "usage_tracker.json")
...
def fetch_h1_tags(url: str):
    """Fetch all H1 tags from a URL with robust HTML detection."""
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        response = requests.get(url, timeout=10, headers=headers)
        response.raise_for_status()
    except requests.RequestException:
        return ["unable to retrieve the html file"]

    content_type = response.headers.get("Content-Type", "")
    if not any(x in content_type.lower() for x in ["text/html", "application/xhtml+xml"]):
        return ["unable to retrieve the html file"]
